...
#load test dataset
obc,obstacles, paths, path_lengths, all_obstacles= load_test_dataset()
...
kdtrees = {}
def IsInCollision(x,idx, radius=1):
	if idx not in kdtrees:
		# build kdtrees.
		print(all_obstacles[idx])
		tree = scipy.spatial.cKDTree(all_obstacles[idx])
		# print(x)
		# print(tree.query_ball_point(x, 2))
		kdtrees[idx] = tree
	tree = kdtrees[idx]
	# print(len(tree.query_ball_point(x, radius)))
	# asdsd
	if len(tree.query_ball_point(x, radius)) > 0:
		return True
	return False
...
def replan_path(p,g,idx,obs):
	waypts_that_are_free=[p[0]]

	waypts_that_are_free = [_p for _p in p[1:-1] if not IsInCollision(_p, idx)]
	# for i in range(1,len(p)-1):
	# 	if not IsInCollision(p[i],idx):
	# 		waypts_that_are_free.append(p[i])
	waypts_that_are_free.append(g)

	new_path=[]
	for i in range(0,len(waypts_that_are_free)-1):
		target_reached=False

		st=waypts_that_are_free[i]
		gl=waypts_that_are_free[i+1]

		if steerTo(st, gl, idx):
			# add to path if the two waypoints are free
			new_path.append(st)
			new_path.append(gl)
		else:
			pA=[st]
...
def _plan(_begin, _target, obs):
	inp = torch.cat((obs, _begin, _target))
	inp = to_var(inp)
	_next_step = mlp(inp)
	_next_step = _next_step.data.cpu()
	return _next_step
	...
	for i in range(0, 4):
		...
		for j in range(0, 10):
			...
			if ok and path_lengths[i][j] > 0:
				...
				# flip flop between start and goal path
				begin = start1
				target = start2
				cur_tree = 0
				for step in range(80):
					# flip so that it will plan from the other end everytime
					if cur_tree == 0:
						begin, target, path_container = start1, start2, path1
					else:
						begin, target, path_container = start2, start1, path2

					# head towards target from begin
					new_target = _plan(begin, target, obs)  # this is the new target
					path_container.append(target)

					# update existing start/goal
					if cur_tree == 0:
						start1 = new_target
					else:
						start2 = new_target
					...
					# flip flop
					cur_tree = 1 if cur_tree == 0 else 0

					target_reached = steerTo(begin, target, i)
					if target_reached:
						break
				total_path += 1
				...
				if target_reached:
					...
					path = lvc(path, i)
					feasible = feasibility_check(path, i)
					if feasible:
						t = time.clock() - tic
						et.append(t)
						fesible_path += 1
						...
						for sp in range(10):
							g = np.zeros(2, dtype=np.float32)
							g = torch.from_numpy(paths[i][j][path_lengths[i][j] - 1])
							path = replan_path(path, g, i,
											   obs)  # replanning at coarse level
							if path == 0:
								break
							path = lvc(path, i)
							feasible = feasibility_check(path, i)
							if feasible:
								...
								if len(path) < 20:
									print ("new_path:")
									print(np.array([_p.numpy() for _p in path]))
									print ("GT path:")
									print(np.array(paths[i][j][:path_lengths[i][j]]))
								else:
									print("path found, dont worry")
								break
					...
					print(all_obstacles[i])
					print(all_obstacles[i].shape)

					plt.scatter(*all_obstacles[i].reshape(-1, 2).T)
					if path != 0:
						plt.plot(*np.array([_p.numpy() for _p in path]).reshape(-1, 2).T, color='red')
					plt.plot(*np.array(paths[i][j][:path_lengths[i][j]]).reshape(-1, 2).T, color='green')
					plt.show()
